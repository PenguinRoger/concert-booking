    ...
    <div class="banner">
        ...
        <div class="card-container">
            @foreach ($concerts as $concert)
                <div class="concert-card">
                    ...
                    @if ($concert->total_tickets_available > 0)
                        ...
                        <button type="button" class="btn btn-primary" data-toggle="modal"
                            data-target="#bookingModal-{{ $concert->id }}">
                            จองตั๋ว
                        </button>
                    @else
                ...
                <div class="modal fade" id="bookingModal-{{ $concert->id }}" tabindex="-1"
                    aria-labelledby="bookingModalLabel-{{ $concert->id }}" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="bookingModalLabel">Book Your Ticket for
                                    {{ $concert->name }}</h5>
                            ...
                            <div class="modal-body-booking-ticket">
                                <form action="{{ route('processBooking', ['concert_id' => $concert->id]) }}"
                                    method="POST">
                                    @csrf
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" id="ticket_type" name="ticket_type"
                                            onchange="showTicketPrice(this)">
                                            @foreach ($concert->tickets as $ticket)
                                                @if ($ticket->quantity_avaliable > 0)
                                                    <option value="{{ $ticket->type }}"
                                                        data-price="{{ $ticket->price }}"
                                                        data-remaining-tickets="{{ $ticket->quantity_avaliable }}">
                                                        {{ $ticket->type }} ({{ $ticket->price }} บาท) -
                                                        จำนวนที่เหลือ: {{ $ticket->quantity_avaliable }}
                                                    </option>
                                                @endif
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="number" class="form-control" id="ticket_quantity"
                                            name="ticket_quantity" required>
                                    </div>
